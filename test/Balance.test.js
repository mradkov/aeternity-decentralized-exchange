...
describe('Balance test', () => {
  ...
  it('should have correct balance after take order', async () => {
    ...
    const inputOrder = {
      ...
      buyToken: token,
      hash:
        '0x6378dda51724bca215ddc353efa47107dd942b67df300b533f8f556caed0ffea',
    };
    ...
    // taker balance
    const tokenTakerBalanceBefore = await getBalanceOf(
      taker,
      taker.addr,
      token,
    );
    ...
    // // maker balance
    const tokenMakerBalanceBefore = await getBalanceOf(
      maker,
      maker.addr,
      token,
    );
    ...
    assert.equal(
      tokenTakerBalanceBefore - takerSellAmount,
      tokenTakerBalanceAfter,
      '1',
    );
    const aeTakerBalanceAfter = await getBalanceOf(taker, taker.addr, 0);

    assert.equal(
      aeTakerBalanceBefore +
        takerReceivedAmount -
        takerReceivedAmount * takerFee,
      aeTakerBalanceAfter,
      '2',
    );
    ...
    assert.equal(
      aeMakerBalanceBefore - takerReceivedAmount,
      aeMakerBalanceAfter,
      '3',
    );
    ...
    assert.equal(
      tokenFeeBalanceBefore + takerSellAmount * makerFee,
      tokenFeeBalanceAfter,
      '4',
    );
    const aeFeeBalanceAfter = await getBalanceOf(owner, owner.addr, 0);
    assert.equal(
      aeFeeBalanceBefore + takerReceivedAmount * takerFee,
      aeFeeBalanceAfter,
      '5',
    );
  });
